...
import { enquirySchema, enquiryStatusSchema } from "../utils/validation";
...
export async function updateEnquiry(req: Request, res: Response) { const input = enquiryStatusSchema.parse(req.body); const data = await prisma.enquiry.update({ where: { id: idParam(req.params.id) }, data: input }); res.json({ success: true, data }); }
