...
export async function createOrder(req: Request, res: Response) { const quotationId = idParam(req.params.quotationId); const quotation = await prisma.quotation.findUnique({ where: { id: quotationId }, include: { items: true } }); if (!quotation || quotation.status !== QuotationStatus.ACCEPTED) throw new HttpError(400, "Only an accepted quotation can create an order"); try { const data = await prisma.salesOrder.create({ data: { orderNo: number("ORD"), quotationId, customerId: quotation.customerId, total: quotation.total, items: { create: quotation.items.map(item => ({ productId: item.productId, quantity: item.quantity, unitPrice: item.unitPrice })) } }, include: includeOrder }); res.status(201).json({ success: true, data }); } catch (error: any) { if (error.code === "P2002") throw new HttpError(409, "This quotation already has a sales order"); throw error; } }
